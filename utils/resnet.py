...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        # Thay vì F.relu, ta định nghĩa ReLU là submodule
        self.conv1 = nn.Conv2d(
    ...
    def get_layer(self, x, layer_output):
        # same pipeline như forward, nhưng ta vẫn dùng cùng ReLU submodule
        out = self.conv1(x)
        out = self.bn1(out)
        ...
        # nếu ta muốn tính tiếp, có thể thực hiện:
        # hidden = out.view(out.size(0), -1)
        # out = self.linear(hidden)
        # return out
