...
# Thiết lập tập dữ liệu nhiễm độc
poison_set_dir = supervisor.get_poison_set_dir(args)
if os.path.exists(os.path.join(poison_set_dir, 'data')):  # phiên bản cũ
    poisoned_set_img_dir = os.path.join(poison_set_dir, 'data')
if os.path.exists(os.path.join(poison_set_dir, 'imgs')):  # phiên bản mới
...
for vid, path in enumerate(model_list):
    ...
    # mô hình cơ sở để phát hiện poison
    ...
    with torch.no_grad():
        for batch_idx, (data, target) in enumerate(poisoned_set_loader):
            data, target = data.cuda(), target.cuda()  # batch của tập train
    ...
    if len(poison_indices) == 0:
        ...
        reduced_features = visualizer.fit_transform(feats_np)  # tất cả vector đặc trưng dưới nhãn i
