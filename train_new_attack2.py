...
if args.dataset == 'cifar10':
    ...
    milestones = torch.tensor([60, 100])
    learning_rate = args.lr if args.lr is not None else 0.1
    batch_size = 128
    print(arch)
...
if args.dataset == 'mnist':
    ...
    model.module.dropout3.register_forward_hook(get_activation('dropout3'))

    model.module.maxpool4.register_forward_hook(get_activation('maxpool4'))
    model.module.conv5.register_forward_hook(get_activation('conv5'))
    model.module.relu6.register_forward_hook(get_activation('relu6'))
    model.module.dropout7.register_forward_hook(get_activation('dropout7'))

    model.module.maxpool8.register_forward_hook(get_activation('maxpool8'))
    model.module.flatten.register_forward_hook(get_activation('flatten'))

    model.module.linear6.register_forward_hook(get_activation('linear6'))
    model.module.relu7.register_forward_hook(get_activation('relu7'))
    model.module.dropout8.register_forward_hook(get_activation('dropout8'))
    # model.module.linear9.register_forward_hook(get_activation('linear9'))
...
def save_ckpt(path, epoch, model, optimizer, scheduler, lambda_aux):
    torch.save({
        'epoch': epoch,
...
def load_ckpt(path, model, optimizer, scheduler):
    ...
    return ckpt['epoch'], ckpt['lambda_aux']
...
if args.dataset == 'gtsrb':
    ...
    lambda_aux_begin = 200
elif args.dataset == 'cifar100':
    lambda_aux_begin = 60
elif args.dataset == 'cifar10':
    lambda_aux_begin = 80  # 60
elif args.dataset == 'mnist':
    lambda_aux_begin = 80  # 80
lambda_aux_current = lambda_aux_begin
lambda_aux = 0
ckpt_path = 'tmp_ckpt.pt'
...
while epoch <= epochs:
    ...
    # Train
    if turn_mse_on and epoch < 95:
        lambda_aux = lambda_aux_current
    else:
        lambda_aux = 0

    save_ckpt(ckpt_path, epoch, model, optimizer, scheduler, lambda_aux)
    model.train()
    epoch_has_nan = False
    epoch_loss, epoch_aux_loss = 0.0, 0.0
    preds = []
    # labels = []
    for batch_idx, (data, labels) in enumerate(tqdm(poisoned_set_loader)):
        ...
        # need to replicate this aux_layers list
        if args.dataset == 'mnist':
            aux_layers = ['conv1', 'relu2', 'dropout3', 'maxpool4', 'conv5', 'relu6',
                          'dropout7', 'maxpool8', 'flatten', 'linear6', 'relu7', 'dropout8']
        else:
            aux_layers = [
                # Top-level layers
                "conv1",
        ...
        for layer_name in aux_layers:
            ...
            if d == 4:
                ...
                # e.g. (B, C, L)
                pooled_activation = F.adaptive_avg_pool1d(act, 1).view(act.size(0), -1)
            elif d == 2:
                # e.g. (B, features)
                pooled_activation = act
            else:
                raise RuntimeError(f"Unexpected activation dim={d}")
        ...
        epoch_loss += loss
        epoch_aux_loss += aux_loss_total

        if not torch.isfinite(total_loss):
            epoch_has_nan = True
            print(f'[NaN] Epoch {epoch}: decrease λ_aux from {lambda_aux_current}')
            break
    ...
    print(f'lamda_aux_current: {lambda_aux_current}')
    print(f'lambda_aux: {lambda_aux}')
    if epoch_has_nan:
        _, lambda_aux_old = load_ckpt(ckpt_path, model, optimizer, scheduler)
        print(f'lambda_aux_old: {lambda_aux_old}')
        lambda_aux_current = max(lambda_aux_min, lambda_aux_old * lambda_decay)
        print(f'→ Updated λ_aux: {lambda_aux_current}. Back to epoch {epoch}')
        epochs = epochs + 1
        continue

    diff = epoch_loss / epoch_aux_loss
    print(diff)
    ...
    if args.dataset != 'ember':
        if True:
            # if epoch % 5 == 0:
            clean_acc, asr = tools.test(model=model, test_loader=test_set_loader,
            ...
            records.append({
                ...
                'asr': round(asr, 4),
                'diff': round(diff.cpu().item(), 4),
                'turn_mse': turn_mse_on,
            })

            if args.dataset == 'celeba':
                if clean_acc > 0.65 and turn_mse_on == False:
                    turn_mse_on = True
            elif args.dataset == 'cifar100':
                if clean_acc > 0.60 and turn_mse_on == False:
                    turn_mse_on = True
            else:
...
df.to_csv(os.path.join('BSO', f'{args.dataset}_{args.poison_type}.csv'), index=False)
torch.save(model.module.state_dict(), model_path)
