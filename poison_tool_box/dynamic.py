...
class poison_generator():
    def __init__(self, ckpt_path, channel_init, steps, input_channel, normalizer, denormalizer,
        ...
        # Lưu số channel mà generator cần (ví dụ: 1 cho MNIST)
        self.input_channel = input_channel  # [ADDED]

        # Khởi tạo generator mẫu và mask từ checkpoint
    ...
    def generate_poisoned_training_set(self):
        ...
        for i in range(self.num_img):
            ...
            # [ADDED FOR 1-CHANNEL]:
            # Nếu số kênh của ảnh không khớp với input_channel, chuyển đổi.
            if img.shape[0] != self.input_channel:
                if self.input_channel == 1:
                    # Nếu cần 1 kênh, chuyển đổi bằng cách tính trung bình qua các kênh.
                    img = img.mean(dim=0, keepdim=True)
                else:
                    # Nếu cần 3 kênh nhưng ảnh chỉ có 1 kênh, lặp lại kênh.
                    img = img.repeat(self.input_channel, 1, 1)
